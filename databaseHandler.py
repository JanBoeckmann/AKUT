...
from instanceGraph import *
from instanceGraphForDGM25 import *
    ...
    def updateRelevant(self):
            ...
            while grids_queue:
                ...
                if actual_grid_to_check not in checked_grids.keys():
                    ...
                    if Polygon(actual_grid_poly).intersects(buildingsAsPolygons[building]):
                        ...
                        #     waterHeight['waterHeight'][actual_grid_to_check] = 1
        ...
        for node in connectedNodes:
            if node in relevantNodes and node in nodes_with_buildings_25:
                # if waterHeight['waterHeight'][node] > 0 and node in nodes_with_buildings_25:
        ...
        for auffangbecken in allAuffangbecken:
            ...
            while grids_queue:
                ...
                if actual_grid_to_check not in checked_grids.keys():
                    ...
                    actual_grid_pos, actual_grid_poly = self.computeGridPolygon(actual_grid_to_check, 25)
                    if Polygon(actual_grid_poly).intersects(auffangbeckenAsPolygons[auffangbecken]):
                        nodes_with_massnahme_25.add(actual_grid_to_check)
        ...
        conn.commit()
        conn.close()
        print("Successfully updated relevant and connected nodes in first step")

        self.updateRelevantSecondIteration(nodes_with_buildings_25, nodes_with_massnahme_25)

    def updateRelevantSecondIteration(self, nodes_with_buildings_25, nodes_with_massnahme_25):
        ...
        floodingTimes, waterAmounts, modGraph, modArea, flooded_nodes = gridInstanceGraph.computeInitialSolution(None)
        # print(flooded_nodes)
        flooded_nodes_set = set(flooded_nodes)
        set_of_25_nodes_to_check = nodes_with_buildings_25 - nodes_with_massnahme_25 #nodes on which there is a Building but no maßnahme

        update_5m_grid = dict()
        update_25m_grid = dict()

        for n25 in set_of_25_nodes_to_check:
            node_flooded = False
            ...
            for i in range(5):
                ...
                for j in range(5):
                    ...
                    if (n5x, n5y) in flooded_nodes_set:
                        node_flooded = True
            if not node_flooded:
                for n5 in remember_5m_nodes:
                    update_5m_grid[n5] = (0, 0, n5[0], n5[1])
                update_25m_grid[n25] = (1, 1, 0, 1, n25[0], n25[1])

        to_db = self.dict_to_array(update_25m_grid)
        to_db_5 = self.dict_to_array(update_5m_grid)
